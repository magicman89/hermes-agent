...
class TestCharacterCountGuard(unittest.TestCase):
    ...
    @patch("tools.file_tools._get_file_ops")
    def test_effective_limit_uses_min_of_filesize_and_max(self, mock_ops):
        """When file_size < _MAX_READ_CHARS, the file_size is the effective cap."""
        # File is 50 bytes, content is 60 chars (bigger than file — impossible
        # in real life but tests the min() logic).
        mock_ops.return_value = _make_fake_ops(content="y" * 60, file_size=50)
        result = json.loads(read_file_tool("/tmp/trick.txt", task_id="trick"))
        self.assertIn("error", result)
        self.assertIn("safety limit", result["error"])
